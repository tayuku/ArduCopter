...
static void auto_run()
{
    ...
    case Auto_TakeOff:
        test_auto_mode_print = 1;
        auto_takeoff_run();
        break;

    case Auto_WP:
    case Auto_CircleMoveToEdge:
        test_auto_mode_print = 2;
        auto_wp_run();
        break;

    case Auto_Land:
        test_auto_mode_print = 3;
        auto_land_run();
        break;

    case Auto_RTL:
        test_auto_mode_print = 4;
        auto_rtl_run();
        break;

    case Auto_Circle:
        test_auto_mode_print = 5;
        auto_circle_run();
        break;

    case Auto_Spline:
        test_auto_mode_print = 6;
        auto_spline_run();
        break;

    case Auto_NavGuided:
        test_auto_mode_print = 7;
    ...
    case Auto_Loiter:
        test_auto_mode_print = 8;
        auto_loiter_run();
        break;

    case Auto_Avoid:
        test_auto_mode_print = 9;
        auto_avoid_run();
        break;
    }
}
...
{
    //************************* * * * * * * * * * *************************//
    int16_t auto_sonar_cm = 0;
    auto_sonar_cm = sonar.distance_cm();
    int16_t auto_sonar_thr_upper= 150;
    int16_t auto_sonar_thr_lower= 30;
    //************************* * * * * $ * * * * *************************//
    ...
    //************************* * * * * * * * * * *************************//
    //quit wp mode if see object
    if (auto_sonar_cm < auto_sonar_thr_upper && auto_sonar_cm > auto_sonar_thr_lower ){
        auto_mode = Auto_Avoid;
        return;
        }
    //************************* * * * * $ * * * * *************************//
...
void auto_avoid_run()
{
    ...
    if (auto_sonar_cm < auto_sonar_thr_upper && auto_sonar_cm > auto_sonar_thr_lower ){
        auto_avoid_force_left();
        }
    else{auto_mode = Auto_WP;
        return;
    }
}
//copy from loiter_run(), replace rc_1.control_in and etc
static void auto_avoid_force_left()
{
	float target_yaw_rate = 0;
    ...
    if (!failsafe.radio) {
        ...
        // process pilot's roll and pitch input
        wp_nav.set_pilot_desired_acceleration(-4500, 0);
    ...
    if (ap.land_complete) {
        ...
        // run altitude controller (surface tracking suspended, sonar used for test)
